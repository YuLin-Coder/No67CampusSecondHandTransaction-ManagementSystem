...
<script type="text/javascript">
	$(function() {
		$("#btnDelete")
				.click(
						function() {
							var ids = $(".check[type=checkbox]:checked")
									.serialize();
							...
							$
									.ajax({
										url : "${pageContext.request.contextPath}/admin/hytypemanager.do?actiontype=delete",
										method : 'post',
										data : ids,
...
<body>
	<div class="search-title">
		...
		<div class="description">
			<a
				href="${pageContext.request.contextPath}/admin/hytypemanager.do?actiontype=load&forwardurl=/admin/hytypeadd.jsp">新建会员级别</a>
		</div>
	</div>
	<!-- 搜索控件开始 -->
	<div class="search-options">
		<form id="searchForm"
			action="${pageContext.request.contextPath}/admin/hytypemanager.do"
			method="post">
	...
	<table id="hytype" width="100%" border="0" cellspacing="0"
		...
		<tbody>
			<c:if test="${listhytype== null || fn:length(listhytype) == 0}">
				<tr>
					<td colspan="20">没有相关会员级别信息</td>
				</tr>
			</c:if>
			<%
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				if (request.getAttribute("listhytype") != null) {
					List<Hytype> listhytype = (List<Hytype>) request.getAttribute("listhytype");
					for (Hytype temhytype : listhytype) {
			%>
			<tr>
				<td>&nbsp<input id="chk<%=temhytype.getId()%>" class="check"
					name="ids" type="checkbox" value='<%=temhytype.getId()%>' /></td>
				<td><%=temhytype.getName()%></td>
				<td><%=temhytype.getDiscount()%>折</td>
				<td><a class="orange-href"
					href="${pageContext.request.contextPath}/admin/hytypemanager.do?actiontype=load&id=<%=temhytype.getId()%>&forwardurl=/admin/hytypeadd.jsp">修改</a>
